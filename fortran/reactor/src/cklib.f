...
C     third-body reactions
C
C	pey -- commented out the following if-statement (always need CTOT) (8/7/04)
C      IF (NTHB .GT. 0) THEN
         CTOT = 0.0
         DO 10 K = 1, KK
            CTOT = CTOT + C(K)
...
C
            PCOR = PR / (1.0 + PR)
C
C	pey -- changed following if statement (8/6/04)
C            IF (IFOP(N) .GT. 1) THEN
            IF ((IFOP(N).GT.1) .AND. (IFOP(N).LT.5)) THEN
               PRLOG = LOG10(MAX(PR,SMALL))
C
               IF (IFOP(N) .EQ. 2) THEN
C
...
C
            ...
            ENDIF
C	begin pey (8/6/04)
            IF (IFOP(N).EQ.5) THEN ! CHEBYSHEV POLYNOMIAL FORM
               ...
               P    = 82.0528D0 * CTOT * T ! ATM

               TCHEB = (2.0D0*(1.0D0/T) - 1.0D0/TMIN - 1.0D0/TMAX) / 
     >                     (1.0D0/TMIN - 1.0D0/TMAX)
	         PCHEB = (2.0D0*LOG10(P) - LOG10(PMIN) - LOG10(PMAX)) / 
     >                     (LOG10(PMAX) - LOG10(PMIN))

               IF (TCHEB.GE.-1.0D0 .AND. TCHEB.LE.1.0D0 .AND. 
     >             PCHEB.GE.-1.0D0 .AND. PCHEB.LE.1.0D0) THEN

                  ALOGK = 0.0D0
                  DO I=1,NT
                     DO J=1,NP
                        AIJ = FPAR((I-1)*NP + J + 9,N)
                        ALOGK = ALOGK + AIJ*PHI(I,TCHEB)*PHI(J,PCHEB)
                     ENDDO
                  ENDDO
                  RKCHEB = 10.0D0**ALOGK

               ELSE
                  WRITE(*,'(2A,I4)') 'Error: T or P out-of-range for', 
     >                ' Chebyshev fit for rxn ', IFAL(N)
	            WRITE(*,*)  'T,P = ', T, P
                  STOP
               ENDIF
c	         WRITE(*,'(3E15.6)') T,P,RKCHEB
            ENDIF
C	end pey

C	begin pey (8/6/04) -- modify for chebyshev option
            IF (IFOP(N).EQ.5) THEN    !Chebyshev polynomial
               RKFTN = RKFT(IFAL(N))
               RKFT(IFAL(N)) = RKCHEB
               RKRT(IFAL(N)) = RKCHEB * RKRT(IFAL(N)) / RKFTN
            ELSE                      ! other pdep formulas
               RKFT(IFAL(N)) = RKFT(IFAL(N)) * PCOR
               RKRT(IFAL(N)) = RKRT(IFAL(N)) * PCOR
            ENDIF
C	end pey
...
C
      ...
      END
C
C Following subroutine appended here 
C by PEY (19 Aug 2001) from cklib v4.5.
C                                                                      C
C----------------------------------------------------------------------C
C                                                                      C
...
200   CONTINUE
      ...
      END
C
C	The following function added to calculate Chebyshev basis
C     functions. (pey 8/6/04)
C                                                                      C
C----------------------------------------------------------------------C
C                                                                      C
