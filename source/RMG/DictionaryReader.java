...
public class DictionaryReader {
    public static void main(String[] args) {
        ...
        System.setProperty("jing.chem.LJDatabase.pathName", workingDir
                + "/databases/RMG_database/transport_groups");
        // block below was commented out and name was changed from inchiDictionaryReader to DictionaryReader after I
// realized biradicals on adjacent were causing difficulty
...
// }
        ...
        // 2. calculate the thermo using QMTP with no HBI; include non-ring species; (primaryThermoLibrary species will
// still not be included); for triplets, use guess=mix and use gaussian?
        try {
            ...
            while (line != null) {
                ...
                if (!line.equals("InChI=1/H") && !line.startsWith("HJ(")
                        && !line.startsWith("H(")) {// {for some reason, H does not seem to work in Gaussian, even
// manually, without freq keyword; not sure about why MOPAC fails
                    ChemGraph chemgraph = ChemGraph.make(g);
                    ReactionModelGenerator
                            .addChemGraphToListIfNotPresent_ElseTerminate(
                                    speciesFromInputFile, chemgraph, "");
                    Species spe = Species.make("molecule", chemgraph);
                    // System.out.println(spe.getName());
                    // calculate and display Lennard-Jones estimates based on Joback correlations for critical
// properties
                    GATransportP LJGAPP = GATransportP.getINSTANCE();
                    TransportData ljdata = LJGAPP
                            .generateTransportData(chemgraph);
                    System.out.println("Sums (dTc, dPc, dVc, dTb): "
                            + ljdata.getGAData().deltaToString());
                    System.out.println("Tc (K) = "
                            + ljdata.getGAData().calculateTc());
                    System.out.println("Pc (bar) = "
                            + ljdata.getGAData().calculatePc());
                    System.out.println("Vc (cc/mol) = "
                            + ljdata.getGAData().calculateVc());
                    System.out.println("Tb (K) = "
                            + ljdata.getGAData().calculateTb());
                    System.out.println("omega = "
                            + ljdata.getGAData().calculateOmega());
                    System.out.println("sigma (Angstroms) = "
                            + ljdata.getSigma());
                    System.out.println("epsilon (K) = " + ljdata.getEpsilon());
                    System.out.println("\n\n");
                    /*
                     * Following line added by MRH on 10Aug2009: After the species is made, the chemgraph is not
                     * necessarily the same (e.g. the species contains resonance isomers, and the adjacency list
                     * supplied by the user is not the most stable isomer). This was causing a discrepancy to be
                     * displayed to the screen: the "H=" value we were displaying corresponded to the chemgraph supplied
                     * by the user; the ThermData corresponded to the most stable isomer. An example of a troublesome
                     * chemgraph: 1 C 0 {2,T} 2 C 0 {1,T} {3,S} 3 O 0 {2,S}
                     */
        ...
        // after the run, unoptimized 3D-mol files should be present in 3Dmolfiles directory and Gaussian results should
// be in InChIDictionaryQMFiles folder
    }
}
