 ...
 *          [1] number (the pressure of interest)
 *  Output: chem.inp file with Arrhenius nomenclature
 *
...
public class ReplacePdepKinetics {
	...
	public static void main(String args[]) {
		...
		try {
			...
			String line = readMeaningfulAndEmptyLine(br_thermodat, false);
			plogChemInpFile += line + "\n";
			
			// Read in chem.inp until REACTIONS is found
			while (!line.equals("REACTIONS")) {
				line = readMeaningfulAndEmptyLine(br_thermodat,false);
				plogChemInpFile += line + "\n";
			}
...
//						plogChemInpFile += wholeLine[0] + wholeLine[1] + "\t" + wholeLine[2] + "\n";
                                                String currentLine = wholeLine[0] + wholeLine[1] + "\t";
						...
						while (line.startsWith("!")) {
                                                ...
                                                temperatureInfo = new StringTokenizer(st_cheb.nextToken());
                                                double PMIN = Double.parseDouble(temperatureInfo.nextToken());
                                                double PMAX = Double.parseDouble(temperatureInfo.nextToken());
...
//						plogChemInpFile += convertCHEB2PLOG(coeffs,nT,nP,TMIN,TMAX,PMIN,PMAX);
                                                currentLine += convertCHEB2PLOG(coeffs,nT,nP,TMIN,TMAX,PMIN,PMAX,Double.parseDouble(args[1]));
                                                plogChemInpFile += currentLine + "\t!Arrhenius fit to " + args[1] + " atm";
					}
					else {
						plogChemInpFile += line + "\n";
					}
...
//                double dummy = Math.log10(Plow) + i * (Math.log10(Phigh) - Math.log10(Plow)) / (P.length-1);
//                P[i] = Math.pow(10, dummy);
//                Pbar[i] = (2*Math.log10(P[i]) - Math.log10(Plow) - Math.log10(Phigh)) / (Math.log10(Phigh) - Math.log10(Plow));
//            }
            P[0] = P_of_interest;
