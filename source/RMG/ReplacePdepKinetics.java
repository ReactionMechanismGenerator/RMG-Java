/**
 * ReplacePdepKinetics.class
 *  Input:  [0] chem.inp file with CHEB p-dep nomenclature
 *  		[1] options ARH for Arrhenius and PLOG for PLOG mechanism
 *          [2] number (the pressure of interest) required for ARH option, for PLOG option the 4 pressures (0.1,1,10,100 atm) are hard coded
 *  Output: chem.inp file with Arrhenius/PLOG nomenclature
 *
 * This program takes a CHEMKIN chem.inp file as input.  All lines until
 *  the REACTIONS line are read-in, saved, and immediately skipped over.
 *  Once the REACTIONS line is read-in, the program looks for the string
 *  "1.0E0 0.0 0.0".
 *
 * NOTE: MRH chose this string as the indication that a CHEB p-dep rate
 *  coefficient is present.  As of ~ May 2011, RMG would produce this
 *  string for all CHEB p-dep kinetics.  If this has changed, please update
 *  this code accordingly.
 *
 * Upon finding this string, the program reads in the relevant parameters
 *  (including the TMAX, TMIN, PMAX, PMIN, nT, nP, and the CHEB coefficients).
 *  These are sent to a function that computes the k(T,P) for a given pressure
 *  (the pressure of interest supplied by the user); the temperatures used
 *  are spaced linearly on an inverse temperature space, spanning 1/TMAX to
 *  1/TMIN, with a total of 8 temperatures.
 *
 * NOTE: The "8" is presently hard-coded.
 *
 * The 8 k(T,P_of_interest) are then fit via least-squares to a modified
 *  Arrhenius format.  Finally, the entire CHEB formatted rate coefficient
 *  in the chem.inp file is replaced with the computed modified Arrhenius
 *  format, including adding a comment that states these parameters were fit
 *  from a CHEB at a certain pressure.
 *
 * This program takes ~10 minutes to re-cast a 3.5MB chem.inp file with
...
public class ReplacePdepKinetics {
	...
	public static void main(String args[]) {
			...
			while (line != null && !line.equals("END")) {
				...
				if (!line.startsWith("!")&& !line.toLowerCase().contains("low") && !line.toLowerCase().contains("troe") && !line.toLowerCase().contains("dup") && !line.toLowerCase().contains("plog") && !line.contains("CHEB")) {
					...
					if (line.contains("1.0E0 0.0 0.0")) {
						// Split the reaction string on (+m), this is done to separate the reactions and products in chebychev reaction
                        String[] wholeLine = line.split("\\(\\+m\\)");
                        
                        
                                                
                        if(args[1].equals("PLOG")){
                        	plogChemInpFile += wholeLine[0] + wholeLine[1] + "\t" + wholeLine[2] + "\n";
                        }
                        ...
                        StringTokenizer temperatureInfo = new StringTokenizer(st_cheb.nextToken());
                        double TMIN = Double.parseDouble(temperatureInfo.nextToken());
                        double TMAX = Double.parseDouble(temperatureInfo.nextToken());
                        
                        currentToken = st_cheb.nextToken(); // PCHEB
                        StringTokenizer pressureInfo = new StringTokenizer(st_cheb.nextToken());
                        double PMIN = Double.parseDouble(pressureInfo.nextToken());
                        double PMAX = Double.parseDouble(pressureInfo.nextToken());
						...
						// Replace these kinetics with pdep arrhenius kinetics
							 if(args[1].equals("PLOG")){
								 ...
								 for (int i=0; i<P.length; i++) {
									 plogChemInpFile += convertCHEB2PLOG(coeffs,nT,nP,TMIN,TMAX,PMIN,PMAX,args[1],P[i])+"\n";
									 // System.out.println(convertCHEB2PLOG(coeffs,nT,nP,TMIN,TMAX,PMIN,PMAX,args[1],P[i]));
								 }
							    }
							 else if(args[1].equals("ARH")){
								 currentLine += convertCHEB2PLOG(coeffs,nT,nP,TMIN,TMAX,PMIN,PMAX,args[1],Double.parseDouble(args[2]));
								 plogChemInpFile += currentLine + "\t!Arrhenius fit to " + args[2] + " atm";
		                        }			
		...
		public static String readMeaningfulAndEmptyLine(BufferedReader p_reader, boolean toTrimOrNotToTrim) {
		...
		public static String convertCHEB2PLOG(double[] a, int nT, int nP,
                ...
                double[] T = new double[8]; double[] Tbar = new double[T.length];
           ...
           //Getting normalized cheb P points
                P[0] = P_of_interest;
                Pbar[0] = (2*Math.log10(P[0]) - Math.log10(Plow) - Math.log10(Phigh)) / (Math.log10(Phigh) - Math.log10(Plow));	
                ...
                for (int i=0; i<T.length; i++) {
                    for (int k=0; k<nT; k++) {
                        for (int l=0; l<nP; l++) {
                            ...
                            logk[i][j] += a[k*nP+l] * phi(Tbar[i],k) * phi(Pbar[j],l);
                        }
                    }
...
//                        "\t/\n";
                ...
                if(option.equals("ARH")){
                	...
                	string2return +=
                            String.format("PLOG / %6.2f %4.3e\t%5.3f\t%8.2f /\t!RMSE of fit %2.2f", P[0],Math.exp(b_matrix.get(0,0)), b_matrix.get(1,0), -GasConstant.getCalMolK()*b_matrix.get(2,0),rmse);	
                }
