/**
 * CalculateReverseRateCoefficients.class
 * Input:  [0] chem.inp file with CHEB p-dep nomenclature
 * 		   [1] Pressure at which you want to calculate the reverse rate coefficient	
 * Output: reverseRateCoefficients.txt file with reverse rate coefficients in Arrhenius form
 * 
 * Note: This will calculate reverse rate coefficients only for PDEP reactions in PLOG and CHEB format
 * Note: For PLOGS it will reverse all the pressures as given in the forward reaction and not use the user defined pressure 
 * Note: The reverse rate constants are valid in range of 300 - 2000 K (beyond this range the k_CHEB have large errors) 
 * 
 * The k values are calculated at 20 temperature points between 2000 and 300 K
 */
...
public class CalculateReverseRateCoefficients {
		...
		try {
			...
			while (!foundThermo) {
				  ...
				  if (line.startsWith("THERMO")) {
						foundThermo = true;
						line = ChemParser.readMeaningfulLine(br_chemfile, true);	// This line contains the global Tmin, Tmax, Tmid
						line = ChemParser.readMeaningfulLine(br_chemfile, true);	// This line should have thermo (or comments)
				  }
			}
				...
				if (!line.startsWith("!")) {
					...
					for (int numLines=0; numLines<2; ++numLines) {
						...
						for (int numcoeffs=0; numcoeffs<5; ++numcoeffs) {
							...
							/*Each coefficient in the NASA polynomial is 15 characters long
							 * hence substring(0,15) is 1st coefficient, substring(15,30) is
							 * second coefficient and so on 
							 */
			...
			while (line != null && !line.equals("END")) {
				...
				// By default we will set boolean reversible as true, this will be only set false only for irreversible reactions
				boolean reversible = true;
				
				/*
				 * If line is a comment or is a line giving additional kinetics
				 * skip over that line, we want to search for lines containing
				 * reaction
				 */
				if (!line.startsWith("!")&& !line.toLowerCase().contains("low") && !line.toLowerCase().contains("troe") && !line.toLowerCase().contains("dup") && !line.toLowerCase().contains("plog") && !line.contains("CHEB")) {
                    ...
                    if (line.contains("1.0E0 0.0 0.0")||line.contains("1.0	0.0	0.0")) {
						...
						if (shortRxnString.contains("(+m)")) {
                            ...
                            while (line.startsWith("!")) {
                            	line = ChemParser.readUncommentLine(br_chemfile);
                            }
                            ...
                            double[] logkf =calculateCHEBrate(R,coeffs,nT,nP,Tmin,Tmax,Pmin,Pmax,Pressure,T);
                            
                            /**
                             * ARHENIUS PARAMETERS OF REVERSE RATE COEFFICIENT
                             */
                            String ReverseRateCoeff ="";
                            ReverseRateCoeff = calculate_reverserate_coeff(T,reactsIndex,prodsIndex,reversible,logkf,fullRxnString,shortRxnString);
                            
                            // Append it to string for writing to file
                            reverseRateCoefficients.append(shortRxnString+ "\t"+ "REV/ "+ReverseRateCoeff +"\n");
						}
	                        ...
	                        while (line != null && line.startsWith("PLOG")) {
	                            ...
	                            // NOTE: The values returned are ln(kf)
	                            double[] logkf = calculateArrheniusrate(R,plog_A,plog_n,plog_E,T);
	                            ...
	                            reverseRateCoefficients.append("PLOG / "+plog_pressure+ " " + ReverseRateCoeff+"\n");
                    ...
                    else if(line.contains("=")&&!line.contains("(+M)")&&!line.contains("+m")&&!line.contains("+M=")){
                    	    rmgRate = true;
                    	    shortRxnString = line;
                    	    st = new StringTokenizer(line);
	...
	private static double[] calculateArrheniusrate(double R, double A, double n,
			double E, double[] T) {
	...
	private static String calculate_reverserate_coeff(double[] T,
        ...
        for (int iii=0; iii<T.length; iii++) {
            ...
            logKeq[iii] = (-H_RT + S_R) + Math.log(Math.pow(((8.314*Temperature/Math.pow(10,5))*Math.pow(10,6)),(numR-numP)));
     ...
     // Computing log fitted rate constants
       ...
       // }
        ...
        string2return =
                String.format("%4.3e\t%5.3f\t%8.2f\t!RMSE of fit %2.2e", Math.exp(b_matrix.get(0,0)), b_matrix.get(1,0), -GasConstant.getCalMolK()*b_matrix.get(2,0),rmse);  
	
        return string2return;
	}

	private static double[] calculateCHEBrate(double R, double[] a, int nT, int nP,
              ...
              for (int i=0; i<T.length; i++) {
                  for (int k=0; k<nT; k++) {
                      for (int l=0; l<nP; l++) {
                          ...
                          logk[i] += a[k*nP+l] * phi(Tbar[i],k) * phi(Pbar[0],l);
                      }
                  }
