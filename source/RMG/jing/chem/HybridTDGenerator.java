...
public class HybridTDGenerator extends TDGenerator {
	...
	/**
	 * Acyclic molecules are estimated through Benson group additivity.
	 * For (poly)cyclic molecules, adequate ring corrections are sought for. If they are not 
	 * available, fall back to QMTP. If QMTP fails, fall back to Benson GA estimates without 
	 * ring corrections. 
	 */
	@Override
	public ThermoData generateThermo(ChemGraph chemGraph) {
		if(chemGraph.isAcyclic()){
			...
			else if(fusedPolycyclic && ((GATP)thermoGAPP).getPolycyclic() == null){
