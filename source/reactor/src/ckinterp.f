...
C     VERSION 3.6
C
C	*** with Chebyshev option for k(T,P) added by pey 7/6/04 ***
C
...
C                   = 4 for SRI form
C				  = 5 for Chebyshev form (pey 7/6/04)
...
C
C	NFAR increased from 8 to 109 to accomodate chebyshev coeffs (pey 7/6/04)
C	Added LMSG to print out error message to separate file (pey 14/6/04)
      PARAMETER (MDIM=50, KDIM=1600, MKDIM=MDIM*KDIM, IDIM=7000,LSYM=16,
     1           NPAR=3, NPIDIM=IDIM*NPAR, NPC=5, NPCP2=NPC+2, MAXTP=3,
     2           NTR=MAXTP-1, NKTDIM=NTR*NPCP2*KDIM, MAXSP=7, MAXTB=10,
     3           NLAR=2, NSIDIM=MAXSP*IDIM, NTIDIM=MAXTB*IDIM,
     4           NLIDIM=NLAR*IDIM, NFAR=109, NFIDIM=NFAR*IDIM,
     5           NTDIM=KDIM*MAXTP, NIDIM=11*IDIM, LIN=15, LOUT=16,
     6           LTHRM=17, LMSG=18, LINC=25, CKMIN=1.0E-3, MAXORD=10, 
...
C	begin pey
	OPEN (LMSG,FORM='FORMATTED',STATUS='UNKNOWN',FILE='chem.message')
C	end pey
C
      VERS = '3.6'
...
C
     ...
     5          NU(MAXSP,*), IRNU(*), RNU(MAXSP,*)
      CHARACTER SUB(*)*(*), KNAME(*)*(*), KEY*80, RSTR*80, UPCASE*5, !UPCASE*4,
     1          ISTR*80
      LOGICAL KERR, LLAN, LRLT, LTHB, LFAL, LTRO, LSRI, LWL, LREV,
     1        LFORD, LRORD
C	begin pey
	COMMON /CHEBYSHEV/ INDXCHB
C	end pey
C
...
C
         ...
         ENDIF
C
C	begin pey (7/6/04) added options for Chebyshev fit (TCHEB,PCHEB,CHEB)
         IF (UPCASE(KEY, 3).EQ.'LOW'  .OR.
     1       UPCASE(KEY, 4).EQ.'TROE' .OR.
     2       UPCASE(KEY, 5).EQ.'TCHEB'.OR.
     3       UPCASE(KEY, 5).EQ.'PCHEB'.OR.
     4       UPCASE(KEY, 4).EQ.'CHEB' .OR. 
     5       UPCASE(KEY, 3).EQ.'SRI') THEN
C	end pey
...
C
               ELSEIF (UPCASE(KEY, 3) .EQ. 'SRI') THEN
                  ...
                  ELSE
                     ...
                     ENDIF
                  ENDIF
C	begin pey (7/6/04) -- read auxiliary keywords TCHEB, PCHEB, and CHEB
C
C     The order in PFAL is "[], [], [], TMIN, TMAX, PMIN, PMAX, N, M, A11, A12,...A1M, A21,...ANM"
C
C               ENDIF
               ELSEIF (UPCASE(KEY, 5) .EQ. 'TCHEB') THEN
	            IFOP(NFAL) = 5 ! indicator for Chebyshev polynomial
	            INDXCHB = 8    ! initialize index
	            CALL IPPARR (RSTR,1,2,PFAL(4,NFAL),NVAL,IER,LOUT)
                  WRITE(LOUT,3096) PFAL(4,NFAL), PFAL(5,NFAL)

               ELSEIF (UPCASE(KEY, 5) .EQ. 'PCHEB') THEN
	            CALL IPPARR (RSTR,1,2,PFAL(6,NFAL),NVAL,IER,LOUT)
                  WRITE(LOUT,3097) PFAL(6,NFAL), PFAL(7,NFAL)

               ELSEIF (UPCASE(KEY, 4) .EQ. 'CHEB') THEN
                  CALL IPPARR (RSTR,1,-8,PFAL(INDXCHB,NFAL),NVAL,
     >                         IER,LOUT)

                  NM = INT(PFAL(8,NFAL)*PFAL(9,NFAL))
                  IF(NM.GT.(NFAR-9)) THEN
                     WRITE(LOUT,'(A)') 'Error: Must increase NFAR'
                     WRITE(LOUT,3098)INT(PFAL(8,NFAL)),INT(PFAL(9,NFAL))
                     STOP
                  ENDIF

                  INDXCHB = INDXCHB + NVAL
	            IF ((INDXCHB-10).LE.NM) THEN
                     WRITE(LOUT,3095)
     >                        (PFAL(L,NFAL),L=INDXCHB-NVAL,INDXCHB-1)
                  ELSE
                    WRITE(LOUT,2015) SUB(N)(:ILEN)
                    KERR = .TRUE.
                  ENDIF
               ENDIF
C	end pey
...
C	begin pey
 2015 FORMAT (6X,'Error in CHEB fall-off parameters...',A)
 3095 FORMAT (6X,'CHEB Polynomials:  ',8E13.5)
 3096 FORMAT (6X,'Tmin, Tmax:  ', 2E13.5)
 3097 FORMAT (6X,'Pmin, Pmax:  ', 2E13.5)
 3098 FORMAT (6X, 'N = ', I4, ' M = ' I4)
C	end pey
...
C
  ...
  500 FORMAT (1X,I4,'. ',A16,T24,A1,T26,I1,T28,F9.5,T38,F6.1,T45,F6.1,
     1       T51,15(I3))
      END
C----------------------------------------------------------------------C
